...
function safeSourceURL(value){try{const u=new URL(value);return u.protocol==='https:'?u.href:'';}catch{return '';}}
function savePersonalWord(word){
 let entry=savedWords.find(w=>w.de===word.de);
 if(!entry){entry={de:word.de,en:word.en,az:word.az,level:word.level||courseLevel,plural:word.plural||'',forms:word.forms||'',note:''};savedWords.push(entry);}
 if(typeof word.context==='string'&&!entry.context)entry.context=word.context.slice(0,2000);
 if(word.sourceURL&&!entry.sourceURL)entry.sourceURL=safeSourceURL(word.sourceURL);
 persistSavedWords();renderSaved();decorateWordCards();showToast(translationLanguage==='az'?'Söz siyahıya əlavə edildi':'Word added to your list');
}
...
function renderSaved(){
 renderSavedSearch();
 $('#savedWordsList').innerHTML=savedWords.length?savedWords.map((w,i)=>`<article class="word-card"><span class="tag">${escapeHtml(w.level)}</span><h3 lang="de">${escapeHtml(w.de)}</h3><p>${escapeHtml(meaning(w))}</p>${w.plural?`<p>${st('plural')}: ${escapeHtml(w.plural)}</p>`:''}${w.forms?`<p>${escapeHtml(w.forms)}</p>`:''}${w.context?`<blockquote class="saved-context" lang="de">${escapeHtml(w.context)}</blockquote>`:''}${safeSourceURL(w.sourceURL)?`<a class="saved-source" href="${escapeHtml(safeSourceURL(w.sourceURL))}" target="_blank" rel="noopener noreferrer">${translationLanguage==='az'?'Mənbə':'Source'} ↗</a>`:''}<button type="button" class="speaker-btn" data-speak="${escapeHtml(w.de)}">♪</button><label class="personal-note">${ct('note')}<textarea data-word-note="${i}" maxlength="800">${escapeHtml(w.note||'')}</textarea></label><button type="button" class="btn secondary" data-remove-personal="${i}">${ct('remove')}</button></article>`).join(''):`<p class="empty-state">${ct('empty')}</p>`;
...
function stopCourseMedia(){if(typeof destroyTranscriptPlayer==='function')destroyTranscriptPlayer();document.querySelectorAll('audio').forEach(a=>a.pause());document.querySelectorAll('.media-frame iframe').forEach(f=>f.remove());document.querySelectorAll('[data-play-media]').forEach(b=>b.hidden=false);}
function showCourse(){
...
function renderMedia(){
 if(typeof destroyTranscriptPlayer==='function')destroyTranscriptPlayer();
...
function playCourseMedia(m,button){
 if(m.youtube&&typeof startTranscriptVideo==='function'){startTranscriptVideo(m,button);return;}
